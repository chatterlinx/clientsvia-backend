...
function signControlPlaneSnapshot(snapshot, opts = {}) {
    ...
    // Audit stamp: include build/deploy info for traceability
    const buildSha = process.env.RENDER_GIT_COMMIT || process.env.GIT_COMMIT || null;
    ...
    logger.info('[SNAPSHOT INTEGRITY] Signed snapshot', {
        ...
        signedAt,
        buildSha: buildSha ? buildSha.substring(0, 8) : null
    });
    ...
    return {
        ...
        _integrity: {
            ...
            companyId,
            snapshotVersion,
            // Audit stamp: which deployed version signed this
            buildSha: buildSha ? buildSha.substring(0, 12) : null,
            env: nodeEnv
        }
    };
}
