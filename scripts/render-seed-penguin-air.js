...
/**
 * ============================================================================
 * RENDER SHELL SEED — Penguin Air Promotions + Voice Settings
 * ============================================================================
 *
 * This version uses the raw `mongodb` driver (no mongoose, no dotenv)
 * so it runs cleanly in the Render Shell where MONGODB_URI is already set
 * as an environment variable.
 *
 * Usage — paste into Render Shell:
 *   node scripts/render-seed-penguin-air.js [companyId]
 *
 * Defaults to Penguin Air's companyId if no arg provided.
...
const PROMOTIONS = [
  // ── 1. Spring Tune-Up — HAS_COUPON → code validation ────────────────────
  {
  ...
  // ── 2. New System Installation — high-value code promo ───────────────────
  {
  ...
  // ── 3. WiFi Thermostat — service-type matching demo ──────────────────────
  //    Fires when caller asks about thermostat promos, even mid-maintenance call
  {
  ...
  // ── 4. Free Diagnostic — no code, ASKING_SPECIALS path ───────────────────
  //    When caller asks "do you have any deals?" this gets listed
  {
...
async function main() {
  console.log('');
  console.log('══════════════════════════════════════════════════════════════');
  console.log('  🎟️  Render Seed: Penguin Air — Promotions + Voice Settings');
  ...
  const promos    = db.collection('companyPromotions');
  const settings  = db.collection('companyPromotionSettings');
  ...
  for (const p of PROMOTIONS) {
    ...
    const result = await promos.findOneAndUpdate(
      { companyId: COMPANY_ID, name: p.name },
  ...
  console.log('  ┌─────────────────────────────────────────────────────────┐');
  console.log('  │  CALLER SAYS         → PATH FIRES                       │');
  console.log('  │  "I have a coupon"   → AMBIGUOUS → clarifying question  │');
  console.log('  │  "code SPRING25"     → HAS_COUPON → validates → $25 off │');
  console.log('  │  "code BADCODE"      → HAS_COUPON → invalid → fallback  │');
  console.log('  │  "any specials?"     → ASKING_SPECIALS → lists 4 promos │');
  console.log('  │  "thermostat deal?"  → service-type → WIFI99 → $99 deal │');
  console.log('  │  "code NEWAC500"     → HAS_COUPON → validates → $500 off│');
  console.log('  │  "is the diag free?" → ASKING_SPECIALS → free diag deal │');
  console.log('  └─────────────────────────────────────────────────────────┘');
  console.log('');
  console.log('  Then refresh promotions.html → you should see 4 promo cards');
  console.log('  + Agent Voice Settings accordion pre-filled with Penguin Air copy.');
  console.log('');
