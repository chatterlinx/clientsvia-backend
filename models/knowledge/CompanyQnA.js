...
companyQnASchema.pre('save', async function(next) {
  try {
    ...
    if (this.isModified('question') || this.isModified('answer') || this.isNew) {
      ...
      console.log('🧠 Generating AI keywords for Q&A:', this.question.substring(0, 50) + '...');
      ...
      this.category = keywordService.detectCategory(this.question, this.answer);
      this.priority = keywordService.detectPriority(this.question, this.answer);
      
      // Update metadata
      this.keywordGenerationVersion = keywordService.getVersion();
      this.autoGenerated = true;
      
      console.log('✅ Generated keywords:', generatedKeywords.primary.slice(0, 5));
    }
    
    next();
  } catch (error) {
    console.error('❌ Keyword generation failed:', error);
    // Don't fail the save if keyword generation fails
    next();
  }
});
