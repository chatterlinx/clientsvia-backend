...
companyQnASchema.pre('save', async function(next) {
  try {
    ...
    if (this.isModified('question') || this.isModified('answer') || this.isNew) {
      ...
      logger.debug('🧠 CHECKPOINT: Starting keyword generation for Q&A:', `${this.question.substring(0, 50)  }...`);
      ...
      try {
        ...
        logger.info('🧠 CHECKPOINT: Company context loaded for keyword generation');
        ...
        logger.info('🧠 CHECKPOINT: Keywords generated successfully');
        ...
        logger.info('✅ CHECKPOINT: Generated keywords:', generatedKeywords.primary?.slice(0, 5) || []);
        
      } catch (keywordError) {
        logger.error('❌ CHECKPOINT: Keyword generation service failed:', keywordError);
        ...
        logger.info('✅ CHECKPOINT: Using fallback keywords:', basicKeywords.slice(0, 5));
      }
    }
    
    logger.info('✅ CHECKPOINT: Pre-save middleware completed successfully');
    next();
    
  } catch (error) {
    logger.error('❌ CRITICAL: Pre-save middleware failed completely:', error);
    logger.error('❌ CHECKPOINT: This will cause 500 error on Q&A save');
...
companyQnASchema.methods.generateFallbackKeywords = function(question, answer) {
  try {
    ...
    logger.error('❌ Even fallback keyword generation failed:', error);
...
companyQnASchema.statics.regenerateAllKeywords = async function(companyId) {
  ...
  for (const qna of qnas) {
    try {
      ...
      logger.error(`Failed to regenerate keywords for Q&A ${qna._id}:`, error);
      results.errors++;
    }
  }
