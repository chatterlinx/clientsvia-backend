...
router.post('/companies', async (req, res) => {
    ...
    const db = getDB();
    if (!db) {
        console.error('[API POST /api/companies] Database not connected');
    ...
    try {
        ...
        const newCompanyData = {
            ...
            // Initialize nested structures to ensure paths exist for future updates
            twilioConfig: { accountSid: null, authToken: null, phoneNumber: null },
            smsSettings: { jobAlerts: false, customerReplies: false, appointmentReminders: false },
            integrations: {
                highlevelApiKey: null,
                highlevelCalendarId: null,
                googleCalendar: { // Default Google Calendar integration structure
                    isLinked: false,
            ...
            aiSettings: { // Default AI settings including the new Google Calendar switch
                ...
                enableGoogleCalendarIntegration: false, // Default to false
                llmFallbackEnabled: false,
                customEscalationMessage: 'I\u2019m unable to answer that, let me connect you to a team member who can help.',
                twilioSpeechConfidenceThreshold: 0.5,
                bargeIn: true,
                humanLikeFillers: false,
                fillerPhrases: [
                    'Let me check that...',
                    'Alright, just a moment...'
                ],
                maxRepeats: 3,
                repeatEscalationMessage: "I'm having trouble understanding. Let me connect you to a team member.",
                silenceTimeout: 5,
                debugMode: false
            },
            agentSetup: { /* Populate with defaults or ensure they are set by Mongoose if using models */ },
            personalityResponses: defaultResponses,
            notes: [],
            isActive: true,
            createdAt: new Date(),
            updatedAt: new Date()
        };
        // To ensure all sub-document defaults from your Mongoose schema are applied,
        // it would be best to instantiate a Mongoose model here and save it.
        // For now, setting explicit defaults for new structures.
        // Example: (if Company is your Mongoose model)
        ...
        if (result.acknowledged && result.insertedId) {
            console.log('[API POST /api/companies] Company added successfully:', newCompanyData.companyName, 'ID:', result.insertedId);
            const createdCompany = await companiesCollection.findOne({ _id: result.insertedId });
            res.status(201).json(createdCompany);
        } else {
            console.error('[API POST /api/companies] Database insert failed. Result:', result);
    ...
    try {
        ...
        if (updates.address && typeof updates.address === 'object') {
            for (const subKey in updates.address) {
                setOperation[`address.${subKey}`] = updates.address[subKey];
            }
            delete updates.address;
        } else {
             if(updates.addressStreet !== undefined) { setOperation['address.street'] = updates.addressStreet; delete updates.addressStreet;}
             if(updates.addressCity   !== undefined) { setOperation['address.city'] = updates.addressCity;   delete updates.addressCity;}
             if(updates.addressState  !== undefined) { setOperation['address.state'] = updates.addressState;  delete updates.addressState;}
             if(updates.addressZip    !== undefined) { setOperation['address.zip'] = updates.addressZip;    delete updates.addressZip;}
             if(updates.addressCountry!== undefined) { setOperation['address.country'] = updates.addressCountry; delete updates.addressCountry;}
        }
        ...
        for (const key in updates) {
            if (Object.prototype.hasOwnProperty.call(updates, key)) {
                if (key === 'tradeTypes') {
                    ...
                    setOperation[key] = updates[key] === 'true' || updates[key] === true;
        ...
        setOperation.updatedAt = new Date();
