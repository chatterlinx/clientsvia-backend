...
(function() {
  ...
  const state = {
    ...
    // ═══════════════════════════════════════════════════════════════
    // 🎙️ GREETINGS STATE (Agent 2.0 Greeting Interceptor)
    // ═══════════════════════════════════════════════════════════════
    greetings: {
      interceptor: {
        enabled: true,
  ...
  function setupEventListeners() {
    ...
    // Setup greeting interceptor listeners
    setupGreetingInterceptorListeners();
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     🎙️ GREETING INTERCEPTOR - Responds to short greetings like "hi", "hello"
     Loaded from agent2 settings, managed inside Name Greeting modal
     ══════════════════════════════════════════════════════════════════════════ */

  async function loadGreetingInterceptor() {
    try {
      console.log('[Greetings] Loading interceptor config...');
      const response = await apiFetch(`/api/admin/agent2/${state.companyId}/greetings`);
  ...
  function renderGreetingInterceptor() {
    // Greeting Interceptor
  ...
  async function handleDeleteRule(e) {
    ...
    try {
      ...
      if (response.success) {
        state.greetings.interceptor.rules = state.greetings.interceptor.rules.filter(r => (r.ruleId || r.id) !== ruleId);
        renderGreetingRules();
  ...
  function setupGreetingInterceptorListeners() {
    // Greeting Interceptor
