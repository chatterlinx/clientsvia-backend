...
Remember: Make every caller feel heard and confident they're in good hands.`;
  ...
  // ═══════════════════════════════════════════════════════════════════
  // V23: TRIAGE BUILDER WITH PRE-FLIGHT CHECK
  ...
  // ═══════════════════════════════════════════════════════════════════
  // V23: BLOCKER UI - When Brain 2 (Scenarios) is empty
  // ═══════════════════════════════════════════════════════════════════
  
  /**
   * Render the blocker UI when no AiCore scenarios are active
   * This enforces the "Golden Rule": Build Brain 2 first, then Brain 1
   */
  renderTriageBuilderBlocked(container, preFlightResult) {
    console.warn('[TRIAGE BUILDER V23] ❌ BLOCKED - No scenarios loaded');
    
    container.innerHTML = `
      <div class="bg-white border-2 border-red-200 rounded-lg shadow-sm overflow-hidden">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-red-700 mt-1">
                Brain 2 (AiCore Scenarios) must be activated first
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="p-8 text-center">
          ...
          <h2 class="text-2xl font-bold text-red-800 mb-4">NO SCENARIOS LOADED</h2>
          
          <p class="text-gray-700 mb-6 max-w-lg mx-auto">
            You must activate <strong>AiCore Templates</strong> before building Triage rules.<br>
            The Triage Builder needs destinations (scenarios) to route calls to.
          </p>
          ...
          <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6 max-w-lg mx-auto">
            <div class="flex items-start">
              ...
              <div class="text-left">
                <h4 class="font-semibold text-yellow-900 mb-1">The "Golden Rule" of V23 Architecture</h4>
                <p class="text-sm text-yellow-800">
                  <strong>Build the Destination (Brain 2), then build the Map (Brain 1).</strong><br>
                  Scenarios define WHAT your AI can handle. Triage rules define HOW to route calls to those scenarios.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-center gap-4">
            <button 
              onclick="cheatSheetManager.navigateToAiCoreTemplates()"
              class="px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition-colors flex items-center"
            >
  ...
  /**
   * Navigate to AiCore Templates tab
   */
  navigateToAiCoreTemplates() {
    // Try to navigate to AiCore Live Scenarios tab
    const aicoreTabs = document.querySelectorAll('.js-aicore-tab');
    for (const tab of aicoreTabs) {
      if (tab.dataset.aicoreTarget === 'live-scenarios' || tab.dataset.aicoreTarget === 'scenarios') {
        tab.click();
        return;
      }
    }
    
    // Fallback: Try main nav
    const mainNavBtns = document.querySelectorAll('.js-main-nav-btn');
    for (const btn of mainNavBtns) {
      if (btn.dataset.target === 'aicore') {
        btn.click();
        // Then try to click the scenarios tab
        setTimeout(() => {
          const scenariosTab = document.querySelector('[data-aicore-target="live-scenarios"]');
    ...
    // Last fallback: alert
    alert('Navigate to: AiCore → Live Scenarios to activate templates');
  }
