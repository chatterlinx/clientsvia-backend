...
async function loadAllRoutes() {
    ...
    try {
        ...
        routes.scenariosBrowserRoutes = await loadRouteWithTimeout('./routes/company/scenariosBrowser', 'scenariosBrowserRoutes'); // 📂 Scenario Browser (RAW/AUDIT separated)
        ...
        routes.globalTemplatesPatchRoutes = await loadRouteWithTimeout('./routes/admin/globalTemplatesPatch', 'globalTemplatesPatchRoutes'); // 🔒 Global Templates Patch (ONLY safe write path for scenarios)
        ...
        routes.returnLaneRoutes = await loadRouteWithTimeout('./routes/admin/returnLane', 'returnLaneRoutes'); // 🚀 Return Lane Admin (V1 - Post-Response Behavior)
        ...
        routes.companyBookingTriggersRoutes = await loadRouteWithTimeout('./routes/admin/companyBookingTriggers', 'companyBookingTriggersRoutes'); // 📅 Booking Triggers (booking-phase keyword/phrase triggers)
        routes.companyPromotionsRoutes    = await loadRouteWithTimeout('./routes/admin/companyPromotions',    'companyPromotionsRoutes');    // 🎟️ Company Promotions (coupons/specials — agent responds at any call turn)
        ...
        routes.kcIntelligenceRoutes       = await loadRouteWithTimeout('./routes/admin/kcIntelligence',       'kcIntelligenceRoutes');       // 🩺 KC Intelligence (health scan, gap mining, conflict map, todo queue)
        routes.kcGapsRoutes              = await loadRouteWithTimeout('./routes/admin/kcGaps',              'kcGapsRoutes');              // 📋 KC Gaps (call gap audit — section gaps, LLM fallbacks, graceful acks)
        routes.kcHealthRoutes            = await loadRouteWithTimeout('./routes/admin/kcHealth',            'kcHealthRoutes');            // 🩻 KC Health (structural config audit — tradeTerms, noAnchor, phraseCore coverage)
        routes.kcGapsResolveRoutes       = await loadRouteWithTimeout('./routes/admin/kcGapsResolve',       'kcGapsResolveRoutes');       // ✅ KC Gap Verify/Resolve (closed-loop: verify phrase routes, mark resolved)
        routes.costConfigRoutes          = await loadRouteWithTimeout('./routes/admin/costConfig',          'costConfigRoutes');          // 💰 Cost Config (per-company LLM/TTS plan pricing — Claude/Groq/ElevenLabs overrides)
        routes.agentLabRoutes             = await loadRouteWithTimeout('./routes/admin/agentLab',             'agentLabRoutes');             // 🧪 Agent Lab (live call testing platform — scenario gen, session X-ray, KC gap feedback)
        ...
        routes.calibrationRoutes = await loadRouteWithTimeout('./routes/admin/calibration', 'calibrationRoutes'); // 📊 UAP Calibration (qaLog stats — Layer 1/2/Unknown hit rates)
...
function registerRoutes(routes) {
    ...
    app.use('/api/admin/global-templates', routes.globalTemplatesPatchRoutes); // 🔒 Global Templates Patch (ONLY safe write path for scenarios)
    ...
    app.use('/api/admin/return-lane', routes.returnLaneRoutes); // 🚀 Return Lane Admin (V1 - Post-Response Behavior)
    ...
    app.use('/api/admin/agent2/company', routes.companyBookingTriggersRoutes); // 📅 Booking Triggers (booking-phase triggers)
    ...
    app.use('/api/admin/agent2/company', routes.kcIntelligenceRoutes);        // 🩺 KC Intelligence (health scan, gap mining, conflict detection, todo queue) — MUST be before companyKnowledge (/:id wildcard would swallow /intelligence)
    app.use('/api/admin/agent2/company', routes.kcGapsRoutes);              // 📋 KC Gaps (call gap audit — section gaps, LLM fallbacks, graceful acks)
    app.use('/api/admin/agent2/company', routes.kcHealthRoutes);            // 🩻 KC Health (structural config audit — read-only)
    app.use('/api/admin/agent2/company', routes.kcGapsResolveRoutes);       // ✅ KC Gap Verify/Resolve (closed-loop gap clearing)
    app.use('/api/admin/agent2/company', routes.costConfigRoutes);          // 💰 Cost Config (per-company plan pricing — Claude/Groq/ElevenLabs overrides)
    app.use('/api/admin/agent2/company', routes.companyKnowledgeRoutes);       // 🧠 Knowledge Base (unified containers — all informational Q&A)
    app.use('/api/admin/agent2/company', routes.agentLabRoutes);             // 🧪 Agent Lab (live call testing — scenario gen, session X-ray, KC gap feedback)
