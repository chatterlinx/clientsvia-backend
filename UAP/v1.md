...
### 23.3 — Predicates (forthcoming)

The semantic law for each of the 7 universal cues — ALLOW / DENY rules that
are mirrored as executable predicates in `routes/admin/cuePhrasesImport.js`
`_semanticLint()` — is tracked in the v1 hardening build:

**See:** `memory/uap-8cue-anchor-hardening.md` — Item 1 ("Tighten
`_validatePatterns`"). When that item ships, §4-10 of this document receives
the updated ALLOW/DENY + predicate references, and this §23.3 stub is replaced
with the live spec.
