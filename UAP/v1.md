...
**Last verified against code: April 23, 2026**
...
> **⚠️ April 2026 NUKE:** Engine Hub + standalone Behavior Cards retired. Any
> reference below to `EngineHubRuntime`, `engineHub`, `standaloneType`, or
> `formatStandaloneBCForPrompt` describes the previous architecture. Flow-level
> tone/DO/DO-NOT rules now live per-company at
> `company.aiAgentSettings.llmAgent.behaviorRules[]` (managed via services.html
> → Behavior tab, rendered by `composeSystemPrompt()`). Category-linked
> Behavior Cards (one per KC category, consumed on KC answer path via
> `BehaviorCardService.forCategory()`) remain live.
...
23. [Architectural Truth — 7 Universal Cues + 1 Trade Index](#23-architectural-truth--7-universal-cues--1-trade-index-april-23-2026)
24. [Anchor Acronym Expansion](#24-anchor-acronym-expansion-april-23-2026)
25. [Trade Index Specificity + Tie-Break](#25-trade-index-specificity--tie-break-april-23-2026)
26. [Unified ROUTE_DECISION qaLog + Observability Event](#26-unified-route_decision-qalog--observability-event-april-23-2026)
27. [Offline Call Replay CLI](#27-offline-call-replay-cli-april-23-2026)
28. [KC_CUE_MISS Telemetry + Dashboard Panel](#28-kc_cue_miss-telemetry--dashboard-panel-april-23-2026)
29. [cuePhrases + tradeVocabulary Drift Monitor](#29-cuephrases--tradevocabulary-drift-monitor-april-23-2026)
30. [Media Streams + Deepgram Nova-3 Direct STT](#30-media-streams--deepgram-nova-3-direct-stt-april-23-2026)
...
## 29. cuePhrases + tradeVocabulary Drift Monitor (April 23, 2026)

**Commit:** Item 9 — `utils/cuePhrasesDriftMonitor.js` (new module),
`index.js` (`/health/drift` route mount + `startDriftMonitor()` startup
call).

### 29.1 — Purpose

The cuePhrases dictionary and trade vocabularies are the foundation of
every call's routing. Silent drift — a botched import, a reverted
backup, an accidental wipe, a token-type collapse — breaks the
CueExtractor for every tenant simultaneously. Until Item 9 the only
feedback channel was "calls start failing." The drift monitor adds a
daily heartbeat that snapshots the dictionary shape, compares it to
the previous snapshot, and logs a warning (or a critical alarm) when
any axis drifts beyond a conservative threshold.

### 29.2 — Design rules (anti-gimmick guardrail)

- **Pure telemetry.** Never blocks, never mutates, never throws.
- **Pattern mirrors `utils/memoryMonitor.js` exactly** (known-good
  model). Fire-and-forget baseline prime, 24h unref'd interval, tiny
  health route handler.
- **In-memory last-snapshot only** — no separate persistence layer.
  A process restart resets the baseline; the first post-restart run is
  always "baseline recorded, no comparison." This is deliberate:
  rolling deploys don't spam spurious alarms.
- **Thresholds are conservative.** Real dictionary growth (admin adds
  50 new patterns) should NOT fire. Catastrophic drops (dictionary
  wipe, token-type collapse, ≥20 % swings) SHOULD fire.
- **No Redis, no cache, no DB writes.** Read-only against
  AdminSettings and `globalHub.tradeVocabularies`.

### 29.3 — Snapshot shape

`getDriftSnapshot()` returns:

```
{
  timestamp:           ISO string,
  cuePhrasesTotal:     integer,
  cuePhrasesByToken:   { requestCue, permissionCue, infoCue,
                         directiveCue, actionCore, urgencyCore,
                         modifierCore },
  tradeVocabTotal:     integer,
  tradeVocabByTrade:   { <tradeKey>: count },
  phraseIntelligenceUpdatedAt:  ISO string | null,
  backupsCount:        integer,
}
```

Source of truth: `AdminSettings.globalHub.phraseIntelligence.cuePhrases`
(for the 7 cue token types) and `AdminSettings.globalHub.tradeVocabularies`
(for trade term counts). Unknown token types on a pattern row are
counted in `total` but not in any `byToken` bucket — this catches
both legitimate rows and malformed imports.

### 29.4 — Thresholds

```
THRESHOLD.cuePhrasesPct  = 0.20   // ±20% total-count drift → warn
THRESHOLD.perTokenPct    = 0.30   // ±30% any single token drift → warn
THRESHOLD.tradeVocabPct  = 0.25   // ±25% trade-term total drift → warn
```

### 29.5 — Levels (`compareDrift(prev, curr)` return value)

- **info** — no drift detected OR baseline-only (no prior snapshot).
  Logged via `logger.info` as "Daily heartbeat."
- **warn** — at least one axis exceeded its threshold in either
  direction. Logged via `logger.warn`.
- **critical** — dictionary total dropped to zero from a non-zero
  baseline (wipe) OR dropped ≥50 %. Logged via `logger.error`.

Critical always wins over warn when both are present. A new token
class emerging (0 → positive) is always at least warn — the extractor
shouldn't be producing unfamiliar token types.

### 29.6 — Endpoint: GET /health/drift

Mounted next to `/health/memory` in `index.js`. On-demand inspection
and CI smoke-check wiring. Returns:

```
{
  ok:          true,
  snapshot:    <current snapshot>,
  baseline:    <in-memory last snapshot> | null,
  comparison:  { level, findings[] },
  thresholds:  THRESHOLD
}
```

Never returns 5xx. If AdminSettings is unavailable, returns
`{ ok: false, reason: 'snapshot-unavailable' }` at 200 so the endpoint
stays useful even in degraded states.

### 29.7 — Anti-regression guarantees

- **Cron is unref'd** — `setInterval(...).unref()` — the timer never
  prevents process exit. Deploys stay clean.
- **All errors are swallowed.** `getDriftSnapshot()` returns `null` on
  any Mongo hiccup; the daily loop treats `null` as "skip this cycle."
- **AdminSettings import is lazy.** If the model file isn't yet
  resolvable when `cuePhrasesDriftMonitor.js` loads (ordering
  accident), a re-require happens inside `getDriftSnapshot()`.
- **Smoke test at `/tmp/test-drift-monitor.js`** — 20/20 tests cover
  `_pctDelta` edges (zero, infinity, positive/negative), token tally
  (malformed rows, unknown token types), trade vocab tally (missing
  `tradeKey`, null terms), every compareDrift code path (baseline,
  no-drift, per-axis warn, critical wipe, critical ≥50% drop, new
  token class, tradeVocab drift, critical-beats-warn precedence), and
  the 3-axis THRESHOLD + 7-class VALID_TOKENS exports.

### 29.8 — Extension path

Today's 3 axes (total, per-token, tradeVocab) are the minimum
meaningful dictionary shape. Future axes that would plug in cleanly
without changing the surface:

- Backup count drift (`globalHub.phraseIntelligenceBackups.length`)
  — warn on unexpected decrease (backups should only ever grow).
- `phraseIntelligenceUpdatedAt` staleness — warn when the dictionary
  has not been touched for >90 days AND the company has active KC
  content, indicating abandonment.
- Per-trade drift inside `tradeVocabByTrade` — warn on any trade
  losing >30 % of its terms.

All three require zero change to consumers — just add a finding to
`compareDrift()` and bump the smoke test.
## 30. Media Streams + Deepgram Nova-3 Direct STT (April 23, 2026)

**Status:** Shipped (C1–C5 of 5). Feature-flagged OFF by default. Per-tenant opt-in.
**Commits:** `d3fd0bfd3` (C1), `538af2a62` (C2), `2cd9fbac1` (C3), `146967ba1` (C4), `44b1d3b73` (C5).
**Build tracker:** `memory/media-streams-build.md`

### 30.1 — Why

Production STT runs through Twilio `<Gather speechModel="auto">`. Live call testing confirmed word-salad transcripts on normal callers and catastrophic failure on accented / elderly speech — the user's primary reliability concern. Twilio Gather's architecture caps us at:

- One-shot per turn, silence-timer endpointing only
- ~150-character keyword budget (Twilio hint string limit)
- Nova-2 at best (Twilio's partnership ceiling)
- Per-turn +600–1000 ms latency (HTTP round-trips)
- Low-confidence fallback is a rescue, not a fix — Twilio often returns *confident wrong words*

This work bypasses Twilio Gather for STT entirely on opted-in tenants. Audio flows directly from Twilio Media Streams to Deepgram Nova-3 over WebSocket, with acoustic+linguistic endpointing and unlimited weighted keyterm boosts.

### 30.2 — End-to-end flow (flag on)

```
Twilio Voice → POST /voice → company.aiAgentSettings.agent2.mediaStreams.enabled?
                                  │
              ┌───────────────────┴─────────┐
              │ YES + circuit closed        │ NO  (default)
              ▼                             ▼
 <Connect><Stream url="wss://.../media-stream">   [existing Gather path — unchanged]
   + CustomParameters {companyId,callSid,from,to}
              │
              ▼
 MediaStreamServer (ws upgrade on same http.Server)
   on 'start' → load company + AdminSettings
              → resolveMediaStreamConfig() (tenant → platform → fallback)
              → resolveKeywords()          (per-tenant + platform + trade vocab)
              → open Deepgram live WS      (Nova-3, mulaw 8kHz, VAD, UtteranceEnd)
              → initializeCall() (v2AIAgentRuntime) + persist state → Redis(EX=4h)
              → OutboundAudioPlayer.play(greeting)
   on 'media' → forward base64 mulaw to DG
   on DG final+UtteranceEnd → TurnLifecycleAdapter emits turn
              → turnLock.then(_runTurn):
                    • Promise.race(CallRuntime.processTurn, 15s timeout)
                    • persist state → Redis
                    • CallTranscriptV2.appendTrace(MS_* events)
                    • OutboundAudioPlayer.play(response) with cancelToken
   on DG SpeechStarted mid-TTS → cancelToken.cancelled=true + Twilio 'clear' event
   on 'stop'/'close'/errors → closeAll; errors also bailToFallback (REST redirect)
```

### 30.3 — Universal fallback invariant

**Every tenant, every call, can always reach Gather.** The Media Streams branch in `/voice` is wrapped in a try/catch; any throw (config error, circuit open, import failure) falls through to the existing Gather handler. Mid-call failures use Twilio REST `client.calls(sid).update({url: '/api/twilio/media-stream/:companyId/fallback', method: 'POST'})` — `<Connect>` is terminal, so only a REST update can rescue the live call. The fallback route redirects into `/api/twilio/v2-agent-respond/:companyId`.

Effective rule: a broken Media Streams path **cannot** take any tenant's calls down. The worst case is one unusual pause followed by normal Gather.

### 30.4 — Platform-wide design principles (locked)

1. **No hardcoded trade vocabulary anywhere.** `DeepgramService.js` historically contained ~20 HVAC keyterms; C1 scrubbed them. Live path reads `company.aiAgentSettings.agent2.speechDetection.keywords[]` merged with `AdminSettings.globalHub.tradeVocabularies[]` matched by `company.trade`, plus platform-global first names — all via `VocabularyResolver.resolveKeywords(company, adminSettings)`.
2. **No hardcoded model defaults in service files.** Default (`nova-3`, `en-US`, `300ms`/`1000ms`) lives in `AdminSettings.globalHub.mediaStreams.default*`. Tenants can override per-company through `company.aiAgentSettings.agent2.mediaStreams.{model,language,endpointingMs,utteranceEndMs}`.
3. **All events carry `companyId`.** Every CallTranscriptV2 trace entry (MS_STREAM_OPENED, MS_DEEPGRAM_CONNECTED, MS_TURN_EMITTED, MS_MIDCALL_FALLBACK, …) is scoped to the tenant. Health endpoint aggregates across tenants.
4. **Circuit breaker is platform-wide.** One Redis key `deepgram:ms:circuit:global` protects every tenant simultaneously — if Deepgram has a partial outage, every opted-in tenant instantly falls back to Gather instead of every call paying the timeout cost. Pattern: 3 consecutive failures in a 5-minute window → circuit opens for 1h (or until manual `forceClose()`).
5. **Feature flag per company, off by default at rollout.** `company.aiAgentSettings.agent2.mediaStreams.enabled` defaults to `false`. Zero behavior change on deploy. Each tenant opts in via admin UI (Speech Pipeline card in `agent2.html`).
6. **Recovery strings come from UI, not code.** Any new fallback message uses `getRecoveryMessage(company, key)`.

### 30.5 — Critical constants & thresholds

| Constant | Value | File |
|---|---|---|
| Platform default model | `nova-3` | `AdminSettings.globalHub.mediaStreams.defaultModel` |
| Platform default language | `en-US` | `AdminSettings.globalHub.mediaStreams.defaultLanguage` |
| Platform default endpointing | `300 ms` | `AdminSettings.globalHub.mediaStreams.defaultEndpointingMs` |
| Platform default utterance-end | `1000 ms` | `AdminSettings.globalHub.mediaStreams.defaultUtteranceEndMs` |
| Per-turn budget | `15000 ms` | `MediaStreamServer.DEFAULT_MAX_TURN_MS` |
| Frame size (Twilio ↔ DG) | `160 bytes` (20ms @ 8kHz ulaw) | `OutboundAudioPlayer.FRAME_BYTES` |
| TTS output format | `ulaw_8000` (native) | `OutboundAudioPlayer.play()` |
| Circuit failure threshold | `3` consecutive | `DeepgramCircuitBreaker.FAILURE_THRESHOLD` |
| Circuit open duration | `3600 s` (1h) | `DeepgramCircuitBreaker.CIRCUIT_TTL_SECONDS` |
| Circuit failure window | `300 s` (5min) | `DeepgramCircuitBreaker.FAILURE_TTL_SECONDS` |
| Health ring buffer size | `1000` samples | `mediaStreamHealthMonitor.RING_CAPACITY` |
| Fallback count window | `24 h` | `mediaStreamHealthMonitor.FALLBACK_WINDOW_MS` |

### 30.6 — What stays identical (critical for rollback safety)

| Subsystem | Changed? |
|---|---|
| KC / UAP / LAP / Booking / Transfer engines | No — receive same `{speechText, confidence, context}` shape |
| DiscoveryNotesService + `discovery-notes:{companyId}:{callSid}` Redis | No |
| Call state Redis (`call:{callSid}`) | No — same TTL=4h, same shape |
| CallerRecognitionService pre-warm | No |
| TTS via v2elevenLabsService + ElevenLabs circuit breaker | No |
| Audio models (TriggerAudio / GreetingAudio / KCResponseAudio) + `/audio-safe/` | No |
| CallTranscriptV2 model | Extended — MS_* kinds documented, schema unchanged (kind is free-form) |
| Every existing Gather in v2twilio.js (20+ sites) | No — universal fallback |

### 30.7 — New code map

| File | Purpose |
|---|---|
| `routes/mediaStream.js` | TwiML bootstrap + mid-call fallback endpoints |
| `services/mediaStream/MediaStreamServer.js` | WS handler + DG broker + turn orchestrator |
| `services/mediaStream/OutboundAudioPlayer.js` | TTS → ulaw_8000 → 160-byte media frames |
| `services/mediaStream/TurnLifecycleAdapter.js` | DG events → engine turn payloads |
| `services/mediaStream/VocabularyResolver.js` | Per-tenant + platform + trade vocab merge |
| `services/mediaStream/ConfigResolver.js` | Tenant → platform → fallback config resolution |
| `services/mediaStream/DeepgramCircuitBreaker.js` | Redis-backed platform-wide kill switch |
| `utils/mediaStreamHealthMonitor.js` | `/health/media-streams` + daily heartbeat |
| `tests/mediaStream/*.test.js` | 8 suites / 101 tests |

### 30.8 — Observability

- **`GET /health/media-streams`** — JSON with `activeWS`, `totalTurns`, `turnLatencyMs.{p50,p95,p99,max,samples}`, `deepgramConnect.{sampleSize,successRate,totalSinceBoot}`, `midcallFallback.{count,windowMs}`, `circuit.{open,failures,reason,thresholds}`, `uptimeHours`.
- **Daily heartbeat** — logged on boot + every 24h. Warns when circuit is open or connect success-rate < 0.8.
- **Per-call trace (CallTranscriptV2)** — every MS_* kind listed in §23.5 of model catalog. Always carries `companyId` + `callSid`.

### 30.9 — Rollback levers

| Action | Effect | How |
|---|---|---|
| Per-tenant disable | That tenant flips back to Gather instantly | Admin UI Speech Pipeline card → Gather → Save |
| Platform emergency kill | Every tenant instantly bypasses Media Streams for 1h | `node -e "require('./services/mediaStream/DeepgramCircuitBreaker').forceOpen('reason')"` in Render Shell |
| Platform restore | Circuit closes, opted-in tenants resume | `node -e "require('./services/mediaStream/DeepgramCircuitBreaker').forceClose()"` |
| Code rollback | Any of C1–C5 commits is independently revertable | `git revert <sha>` (C2–C5 revert cleanly; C1 reverting would restore HVAC-hardcoded keywords) |

### 30.10 — Rollout protocol

1. Deploy with all tenants OFF (default). Smoke: `/health/media-streams` returns valid JSON, `/health/memory` unchanged.
2. Flip flag for ONE dev tenant. Make 3 test calls (clear, accented, whispering). Compare transcripts to Gather baseline.
3. After dev validation, flip for one production tenant. Monitor `/health/media-streams` for 24 h. Verify `midcallFallback.count < 1%` of turns.
4. After one week of stable operation, enable for additional tenants one at a time.

### 30.11 — Cross-References

- **Build runbook:** `memory/media-streams-build.md` — C1–C5 commit summary, tenant opt-in checklist, troubleshooting.
- **CallTranscriptV2 kind catalog:** `models/CallTranscriptV2.js` (top comment on `TraceSchema`).
- **Circuit breaker pattern origin:** `services/v2elevenLabsService.js:10-77` (same Redis-TTL + in-memory-fallback shape).
- **Health endpoint pattern origin:** `utils/memoryMonitor.js`.
