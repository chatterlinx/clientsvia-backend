...
// ============================================================================
// POST /api/admin/ai-test/:companyId/feedback
// Store feedback on AI responses for learning/improvement
// ============================================================================
router.post('/:companyId/feedback', authenticateJWT, async (req, res) => {
    try {
        const { companyId } = req.params;
        const { 
            sessionId, 
        ...
        logger.info('[AI TEST] Feedback received', {
            companyId,
            rating,
            source,
            reason: reason || 'none'
        ...
        // Store in BlackBox for analysis
        if (BlackBoxLogger) {
            await BlackBoxLogger.logEvent({
                callId: sessionId,
                companyId,
                type: rating === 'good' ? 'AI_RESPONSE_APPROVED' : 'AI_RESPONSE_REJECTED',
                data: {
        ...
        // Also store in a dedicated collection for easy retrieval
        // This can be used later for few-shot examples in prompts
        ...
        res.json({ 
            success: true, 
            message: rating === 'good' 
                ? 'Response saved as good example!' 
                : 'Feedback recorded for improvement'
        });
        
    } catch (error) {
        logger.error('[AI TEST] Feedback error:', error);
        // Don't fail the request even if storage fails
        res.json({ success: true, message: 'Feedback noted (storage unavailable)' });
    }
});

// ============================================================================
// GET /api/admin/ai-test/:companyId/good-examples
// Get approved responses for few-shot learning
// ============================================================================
router.get('/:companyId/good-examples', authenticateJWT, async (req, res) => {
    try {
        const { companyId } = req.params;
        const { limit = 10 } = req.query;
        
        const AIFeedback = require('../../models/AIFeedback');
        const goodExamples = await AIFeedback.find({
            companyId,
            rating: 'good'
        })
        .sort({ createdAt: -1 })
        ...
        res.json({
            success: true,
            examples: goodExamples.map(ex => ({
                userMessage: ex.userMessage,
                aiResponse: ex.aiResponse,
                source: ex.source
            })),
            count: goodExamples.length
        });
        
    } catch (error) {
        logger.error('[AI TEST] Good examples error:', error);
        res.json({ success: true, examples: [], count: 0 });
    }
});
