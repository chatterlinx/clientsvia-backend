...
/**
 * ==============================================================
 * 🎯 COMPANY Q&A - CATEGORY-STYLE ENDPOINTS (ADAPTER LAYER)
 * ==============================================================
 * These endpoints provide a category-like interface for Company Q&As
 * while using the flat CompanyKnowledgeQnA collection underneath.
 * This allows the Global Trade Categories UI to work with Company Q&As.
 */

/**
 * GET /api/company/:companyId/knowledge-management/company-qna (with categories structure)
 * Load all Company Q&As grouped as a single "Company Knowledge" category
 */
router.get('/:companyId/knowledge-management/company-qna', authenticateJWT, async (req, res) => {
    const startTime = Date.now();
    const { companyId } = req.params;
    const { includeQnAs, includeStats } = req.query;

    try {
        logger.info(`📚 GET company Q&A (category-style) request for company ${companyId}`);

        // Load all Company Q&As
        const companyQnAs = await CompanyKnowledgeQnA.find({ companyId }).lean();
        
        // Create a single "Company Knowledge" category containing all Q&As
        const categoryData = [{
            _id: `company-knowledge-${companyId}`,
            name: 'Company Knowledge',
            description: 'Company-specific questions and answers',
            companyId: companyId,
            qnas: companyQnAs.map(qna => ({
                _id: qna._id.toString(),
                id: qna._id.toString(),
                question: qna.question,
                answer: qna.answer,
                keywords: qna.keywords || [],
                autoGenerated: qna.keywords || [],
                manualKeywords: [],
                confidence: qna.confidence || 0.8,
                status: qna.status || 'active',
                createdAt: qna.createdAt,
                updatedAt: qna.updatedAt
            })),
            statistics: {
                totalQnAs: companyQnAs.length,
                totalKeywords: companyQnAs.reduce((sum, q) => sum + (q.keywords?.length || 0), 0),
                lastUpdated: companyQnAs.length > 0 ? Math.max(...companyQnAs.map(q => new Date(q.updatedAt).getTime())) : new Date()
            },
            isActive: true,
            createdAt: new Date()
        }];
        ...
        const responseTime = Date.now() - startTime;
        logger.info(`📚 GET company Q&A (category-style) success`, { 
            responseTime,
            qnAsFound: companyQnAs.length
        });
        ...
        const responseTime = Date.now() - startTime;
        logger.error(`❌ GET company Q&A (category-style) failed`, {
            error: error.message,
            responseTime
        });
 ...
 * Create a "category" (not actually used, just returns success for UI compatibility)
 */
router.post('/:companyId/knowledge-management/company-qna/categories', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 POST company Q&A category request (adapter)`, { companyId, name });

        // For Company Q&A, we don't actually create categories
        // Just return success to keep the UI happy
        const responseTime = Date.now() - startTime;

        res.status(201).json({
            success: true,
            message: 'Category structure not needed for Company Q&A',
            data: {
                _id: `company-knowledge-${companyId}`,
                name: 'Company Knowledge',
                description: 'All company Q&As are stored in a single collection',
                companyId: companyId,
                qnas: [],
                statistics: {
                    totalQnAs: 0,
            ...
            meta: {
                responseTime,
                note: 'Company Q&As use a flat structure - categories are virtual'
            }
        });
 ...
 * Add Q&A (ignores categoryId since we use flat structure)
 */
router.post('/:companyId/knowledge-management/company-qna/categories/:categoryId/qna', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 POST company Q&A (via category endpoint)`, { companyId });
        ...
        // Create Q&A using existing model
        const newQnA = new CompanyKnowledgeQnA({
            companyId,
            question: question.trim(),
            answer: answer.trim(),
            keywords: manualKeywords.map(k => k.trim().toLowerCase()),
            confidence: 0.8,
            status: 'active',
            category: 'general'
        });
        ...
        const responseTime = Date.now() - startTime;
        logger.info(`📚 POST company Q&A success`, { responseTime, qnaId: savedQnA._id });

        res.status(201).json({
            success: true,
            message: 'Q&A created successfully',
            data: {
                categoryName: 'Company Knowledge',
                qna: {
                    _id: savedQnA._id.toString(),
                    question: savedQnA.question,
                    answer: savedQnA.answer,
                    keywords: savedQnA.keywords,
                    confidence: savedQnA.confidence,
...
router.put('/:companyId/knowledge-management/company-qna/categories/:categoryId/qna/:qnaId', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 PUT company Q&A (via category endpoint)`, { companyId, qnaId });

        const updatedQnA = await CompanyKnowledgeQnA.findOneAndUpdate(
            { _id: qnaId, companyId },
            {
                question: question.trim(),
                answer: answer.trim(),
...
router.delete('/:companyId/knowledge-management/company-qna/categories/:categoryId/qna/:qnaId', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 DELETE company Q&A (via category endpoint)`, { companyId, qnaId });

        const deletedQnA = await CompanyKnowledgeQnA.findOneAndDelete({
            _id: qnaId,
            companyId
        });
 ...
 * Delete category (not applicable for flat structure, returns success)
 */
router.delete('/:companyId/knowledge-management/company-qna/categories/:categoryId', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 DELETE company Q&A category (adapter)`, { companyId });

        const responseTime = Date.now() - startTime;

        res.json({
            success: true,
            message: 'Category structure not used for Company Q&A',
            data: {
                categoryName: 'Company Knowledge'
            },
            meta: {
 ...
 * Generate Q&As (adapter - returns message that this feature is not applicable)
 */
router.post('/:companyId/knowledge-management/company-qna/categories/:categoryId/generate-top-qnas', authenticateJWT, async (req, res) => {
    ...
    try {
        logger.info(`📚 POST generate Q&As (adapter)`, { companyId });

        const responseTime = Date.now() - startTime;

        res.json({
            success: true,
            message: 'Use the AI-Powered Q&A Generator in the purple section instead',
            data: {
                categoryName: 'Company Knowledge',
                generatedQnAs: 0,
                totalQnAs: await CompanyKnowledgeQnA.countDocuments({ companyId }),
                skippedDuplicates: 0
            },
            meta: {
                responseTime,
                note: 'Company Q&As use the AI generator - not template-based generation'
            }
        });
