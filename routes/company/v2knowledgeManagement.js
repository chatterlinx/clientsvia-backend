...
router.get('/:companyId/knowledge-management', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        if (!knowledge) {
            ...
            logger.debug(`📚 Cache miss - loading knowledge from DB for company ${companyId}`);
...
router.post('/:companyId/knowledge-management/company-qna', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        // Add to company's knowledge management
        const company = await Company.findByIdAndUpdate(
            companyId,
            {
                $push: { 'aiAgentLogic.knowledgeManagement.companyQnA': newQnA },
                $set: { 
        ...
        const responseTime = Date.now() - startTime;
        logger.info(`📚 POST company Q&A success for company ${companyId}`, {
            responseTime,
            qnaId: newQnA.id,
            keywords: newQnA.keywords.length
        });

        res.status(201).json({
            success: true,
            message: 'Company Q&A created successfully',
            data: newQnA,
            meta: {
                responseTime,
