...
router.post('/:id/qna', async (req, res) => {
    try {
        ...
        // For global trade categories, Q&As are stored in CompanyQnA collection (consistency with GET/PUT)
        const CompanyQnA = require('../models/knowledge/CompanyQnA');
        
        console.log('🔧 ADD Q&A: Creating new Q&A', {
            categoryId: id,
        ...
        // Use the same logic as PUT endpoint for consistency
        ...
        if (hasManualKeywords) {
            ...
            var finalManualKeywords = manualKeywords;
            ...
            var finalManualKeywords = [];
            console.log('🔧 ADD Q&A: Generated auto keywords:', autoKeywords.length);
        }
        
        // Create Q&A in CompanyQnA collection
        const newQnA = new CompanyQnA({
            companyId: 'global', // Global trade category Q&As
            question: question.trim(),
            answer: answer.trim(),
            keywords: finalKeywords,
            autoGenerated: !hasManualKeywords,
            manualKeywords: finalManualKeywords,
            tradeCategories: [category.name], // Associate with trade category
            status: 'active',
            priority: 'normal'
        });
...
router.put('/:id/qna/:qnaId', async (req, res) => {
    try {
        ...
        const updateResult = await CompanyQnA.findByIdAndUpdate(qnaId, {
            ...
            answer: answer.trim(),
            keywords: finalKeywords,
            autoGenerated: !hasManualKeywords, // Boolean: true if auto-generated, false if user-specified
            manualKeywords: finalManualKeywords,
            updatedAt: new Date()
        }, { new: true });
        
        if (updateResult) {
            console.log('✅ EDIT Q&A: Successfully updated', {
                categoryId: id,
                qnaId,
                totalKeywords: finalKeywords.length,
                keywordSource: hasManualKeywords ? 'user-specified' : 'auto-generated',
                savedKeywords: updateResult.keywords,
                savedManualKeywords: updateResult.manualKeywords,
                savedAutoGenerated: updateResult.autoGenerated
            });
...
router.delete('/:id/qna/:qnaId', async (req, res) => {
    try {
        ...
        // For global trade categories, Q&As are stored in CompanyQnA collection, not embedded
        // So we need to delete from CompanyQnA collection directly
        const CompanyQnA = require('../models/knowledge/CompanyQnA');
        
        const deleteResult = await CompanyQnA.findByIdAndDelete(qnaId);
